...
object FunDemo1 {
  ...
  val smaller: Int => Int => Int = x => (y => {
    if(x < y) x else y
  })
  // returns the smaller of the two arguments
  
  val larger: Int => Int => Int = x => y => {
    if(y > x) y else x
  }
  // returns the larger of the two arguments
  
  val balance: Int => Int => Int = x => y => {
  ...
  val dblThenInc: Int => Int = inc compose dbl //Right to left
  val sqrThenTriple: Int => Int = sqr andThen mul(3) //Left to right
  ...
  */
  ...
  def main(args: Array[String]): Unit = {
     ...
     */
    println (inc(3))
    val smallerThanTen = smaller(9)
    println(smallerThanTen(10))
    println(smaller(100)(99))
